...
def export_two_parts_to_excel(
    sql1,
    output_path,
    sql2=None,                     # 改为可选，支持单部分导出
    header_mapping1=None,
    header_mapping2=None,
    fillna_dict1=None,             # 第一部分空值填充字典
    fillna_dict2=None,             # 第二部分空值填充字典
    batch_size1=1000,
    batch_size2=1000,
    max_batches=100,
    sheet_name='Sheet1'
    ...
    参数:
        ...
        fillna_dict1: 第一部分空值填充字典 {列名（映射后）: 填充值}
        fillna_dict2: 第二部分空值填充字典 {列名（映射后）: 填充值}
    ...
    try:
        ...
        df2 = pd.DataFrame()   # 默认为空
        if sql2:               # 仅当传入sql2时才查询第二部分
            print("开始分批查询第二部分...")
...
if __name__ == "__main__":
    ...
    # 示例SQL - 第二部分（可注释掉或传None，则只导出第一部分）
    sql_part2 = """
        SELECT
            RECORD_DETAILS,
    ...
    # 空值默认填充示例：将“银行账户”列的空值填为“未知账户”，金额空值填0
    fillna1 = {
        '银行账户': '未知账户',
        '金额': 0
    }
    ...
    # 调用导出函数（如果要单部分导出，只需传 sql2=None）
    export_two_parts_to_excel(
        sql1=sql_part1,
        output_path=output_path,
        sql2=sql_part2,                    # 若不需要第二部分，改为 None 或注释掉
        header_mapping1=mapping1,
        header_mapping2=mapping2,
        fillna_dict1=fillna1,              # 新增默认值填充
